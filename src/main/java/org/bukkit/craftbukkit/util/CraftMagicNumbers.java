...
public final class CraftMagicNumbers implements UnsafeValues {
    ...
    static {
        for (Block block : net.minecraft.util.registry.Registry.BLOCK) {
            BLOCK_MATERIAL.put(block, Material.getMaterial(net.minecraft.util.registry.Registry.BLOCK.getId(block).getPath().toUpperCase(Locale.ROOT)));
        }

        for (Item item : net.minecraft.util.registry.Registry.ITEM) {
            ITEM_MATERIAL.put(item, Material.getMaterial(net.minecraft.util.registry.Registry.ITEM.getId(item).getPath().toUpperCase(Locale.ROOT)));
        }

        for (net.minecraft.fluid.Fluid fluid : net.minecraft.util.registry.Registry.FLUID) {
            FLUID_MATERIAL.put(fluid, Registry.FLUID.get(CraftNamespacedKey.fromMinecraft(net.minecraft.util.registry.Registry.FLUID.getId(fluid))));
        }
        ...
        for (Material material : Material.values()) {
            ...
            net.minecraft.util.registry.Registry.ITEM.getOrEmpty(key).ifPresent((item) -> {
                MATERIAL_ITEM.put(material, item);
            });
            net.minecraft.util.registry.Registry.BLOCK.getOrEmpty(key).ifPresent((block) -> {
                MATERIAL_BLOCK.put(material, block);
            });
            net.minecraft.util.registry.Registry.FLUID.getOrEmpty(key).ifPresent((fluid) -> {
